...
class CodeGenerator():
    ...
    def __init__(self, openapi_file_name: str, output_dir: str | None = None):
        self.output_dir = output_dir
        temporary_filename: str = "temporary_model.py"
        self._generate_temporary_file(temporary_filename, openapi_file_name)
        source_code = self._import_temporary_file(temporary_filename)
        self._imports = self._extract_imports(source_code)
        self._classes = self._extract_classes(source_code)
        self._source_code = source_code
        # os.remove(temporary_filename)

    def filter_import_node(self, import_node: ast.Import | ast.ImportFrom, used_imports: set[str]):
    ...
    def _generate_temporary_file(self, temporary_filename: str, openapi_file_name: str):
        subprocess.run(
            f"datamodel-codegen  --input {openapi_file_name} --input-file-type openapi "
            f"--output {temporary_filename} --use-union-operator --formatters black"
        )
